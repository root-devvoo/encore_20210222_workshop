...
select * from emp2 where ename = lower('adams');
...
조회결과에서 제외시킨다.
*/
select deptno, min(sal) 최소급여, max(sal) 최대급여 from emp2
group by deptno
order by deptno;
-- 5. 10, 20번 부서에 속해있으면서 급여가 2000 이상인 직원의 이름, 급여, 업무, 부서번호를 조회하시오
select ename, sal, job, deptno from emp2
...
*/
select ename, hiredate, sal, deptno from emp2
